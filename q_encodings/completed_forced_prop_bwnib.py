# Irfansha Shaik, 24.11.2022, Aarhus.
  ...
  def generate_final_gate(self):
    ...
    for i in range(self.parsed.depth-1):
      ...
      if (reverse_index%2==1):
        ...
        if(self.makermaker_game == 1):
          # if white if illegal or game stopped then we propagate to the last:
          ...
          white_illegal_or_stopped_propoagation_gate = self.gates_generator.output_gate

          # only when valid and stopped we check the goal condition:
          self.gates_generator.and_gate([valid_move_output_gate,self.move_variables[reverse_index][5][0]])
          stopped_valid_gate = self.gates_generator.output_gate
          self.gates_generator.if_then_gate(stopped_valid_gate, self.white_goal_output_gate)
          stopped_implication_gate = self.gates_generator.output_gate
          # conjunction with this round of constraints:
          self.gates_generator.and_gate([self.transition_step_output_gates[reverse_index], not_stopped_implication_gate, white_illegal_or_stopped_propoagation_gate, stopped_implication_gate])
        else:
          # if white if illegal then we propagate to the last:
          self.gates_generator.if_then_gate(-valid_move_output_gate, complete_propogation_gate)
          white_illegal_propoagation_gate = self.gates_generator.output_gate

          # conjunction with this round of constraints:
          self.gates_generator.and_gate([self.transition_step_output_gates[reverse_index], not_stopped_implication_gate,white_illegal_propoagation_gate])
