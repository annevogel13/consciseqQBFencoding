...
class TicTacToe:
    ...
    def generate_quantifier_blocks(self):
        ...
        for i in range(self.parsed.depth):
            ...
            if i % 2 == 0:
                self.quantifier_block.append(
                    [
                        "exists("
                        + ", ".join(str(x) for x in self.move_variables[i])
                        + ")"
                    ]
                )
            else:
        ...
        self.quantifier_block.append(
            ["exists(" + ", ".join(str(x) for x in all_goal_vars) + ")"]
        )

        self.quantifier_block.append(
            ["# black boolean variables for determining shape of line: "]
        )
        self.quantifier_block.append(
            [
                "exists("
                + ", ".join(str(x) for x in self.black_position_boolean_vars)
                + ")"
            ]
        )
        ...
        for vars in self.forall_white_goal_path_variables:
            all_goal_vars.extend(vars)
        self.quantifier_block.append(
            ["forall(" + ", ".join(str(x) for x in all_goal_vars) + ")"]
        )
        ...
        for each_position_vars in self.white_goal_neighbours:
            for each_nieghbour in each_position_vars:
                self.quantifier_block.append(
                    ["exists(" + ", ".join(str(x) for x in each_nieghbour) + ")"]
                )
        ...
        for each_position_vars in self.white_goal_boolean_vars:
            self.quantifier_block.append(
                ["exists(" + ", ".join(str(x) for x in each_position_vars) + ")"]
            )
        ...
        self.quantifier_block.append(
            [
                "forall("
                + ", ".join(str(x) for x in self.forall_position_variables)
                + ")"
            ]
        )
        ...
        for i in range(self.parsed.depth + 1):
            self.quantifier_block.append(
                [
                    "exists("
                    + ", ".join(str(x) for x in self.predicate_variables[i])
                    + ")"
                ]
            )
        ...
        self.quantifier_block.append(
            ["# Exists down, side, diagonal and antidiagonal variables: "]
        )
        self.quantifier_block.append(
            ["exists(" + ", ".join(str(x) for x in self.boolean_vars) + ")"]
        )
        ...
        for i in range(4):
            self.quantifier_block.append(
                ["exists(" + ", ".join(str(x) for x in self.neighbours[i]) + ")"]
            )

    def generate_black_transition(self, time_step):
        self.encoding.append(
            [
                "# Player 1 (black) transition function for time step "
                + str(time_step)
                + ": "
            ]
        )
        ...
        self.gates_generator.complete_equality_gate(
            self.move_variables[time_step], self.forall_position_variables
        )
        ...
        self.gates_generator.and_gate(
            [
                -self.predicate_variables[time_step][0],
                self.predicate_variables[time_step + 1][0],
                -self.predicate_variables[time_step + 1][1],
            ]
        )
        ...
        self.gates_generator.complete_equality_gate(
            self.predicate_variables[time_step], self.predicate_variables[time_step + 1]
        )
        self.encoding.append(
            [
    ...
    def generate_white_transition(self, time_step):
        self.encoding.append(
            [
                "# Player 2 (white) transition function for time step "
                + str(time_step)
                + ": "
            ]
        )
        ...
        if (
            self.parsed.args.forall_move_restrictions == "in"
            and self.parsed.num_available_moves
            != int(math.pow(2, self.num_move_variables))
        ):
            # White move restriction:
            self.encoding.append(
                [
                    "# Move constraints (if not powers of 2 or simply restricting moves) :"
                ]
            )
            ...
            lsc.add_circuit(
                self.gates_generator,
                self.move_variables[time_step],
                self.parsed.num_available_moves,
            )
        ...
        self.encoding.append(["# In time step i, occupied must be false:"])
        self.gates_generator.and_gate(
            [equality_output_gate, -self.predicate_variables[time_step][0]]
        )
        ...
        if (
            ...
            self.encoding.append(
                ["# Conjuction for move restriction and above conjunction constraints:"]
            )
            self.gates_generator.and_gate(
                [move_restriction_output_gate, self.gates_generator.output_gate]
            )
    ...
    def generate_initial_gate(self):
        ...
        if len(self.parsed.black_initial_positions) != 0:
            ...
            for position in self.parsed.black_initial_positions:
                binary_format_clause = self.generate_binary_format(
                    self.forall_position_variables, position
                )
            ...
            self.gates_generator.and_gate(
                [self.predicate_variables[0][0], -self.predicate_variables[0][1]]
            )
            self.gates_generator.if_then_gate(
                black_final_output_gate, self.gates_generator.output_gate
            )
        ...
        if len(initial_step_output_gates) != 0:
            ...
            if (
                len(self.parsed.black_initial_positions) == 0
                and len(self.parsed.white_initial_positions) == 0
            ):
                initial_step_output_gates.append(-self.predicate_variables[0][0])
            else:
                self.gates_generator.or_gate(step_or_gates)
                self.gates_generator.or_gate(
                    [self.gates_generator.output_gate, -self.predicate_variables[0][0]]
                )
    ...
    def generate_goal_gate(self):
        ...
        for i in range(self.parsed.num_positions):
            binary_format_clause = self.generate_binary_format(
                self.forall_position_variables, i
            )
            self.gates_generator.and_gate(binary_format_clause)
            ...
            for j in range(4):
                ...
                if neighbour == "na":
                    ...
                    self.gates_generator.if_then_gate(
                        if_condition_output_gate, -self.boolean_vars[j]
                    )
                    neighbour_output_gates.append(self.gates_generator.output_gate)
                else:
                    temp_binary_format_clause = self.generate_binary_format(
                        self.neighbours[j], neighbour
                    )
                    ...
                    self.gates_generator.and_gate(
                        [step_neighbour_gate, self.boolean_vars[j]]
                    )
                    step_then_condition_output_gate = self.gates_generator.output_gate
                    self.encoding.append(
                        ["# if the equality is true, then the conjunction is true: "]
                    )
                    self.gates_generator.if_then_gate(
                        if_condition_output_gate, step_then_condition_output_gate
                    )
                    neighbour_output_gates.append(self.gates_generator.output_gate)
        ...
        self.encoding.append(
            ["# one of the black position boolean variables is true : "]
        )
        ...
        for j in range(4):
            ...
            for i in range(self.safe_max_path_length - 1):
                ...
                self.gates_generator.complete_equality_gate(
                    self.goal_path_variables[i], self.forall_position_variables
                )
                ...
                self.gates_generator.complete_equality_gate(
                    self.goal_path_variables[i + 1], self.neighbours[j]
                )
                ...
                self.gates_generator.and_gate(
                    [self.predicate_variables[-1][0], -self.predicate_variables[-1][1]]
                )
                ...
                self.gates_generator.and_gate(
                    [
                        path_neighbour_equality_output_gate,
                        constraints_output_gate,
                        self.boolean_vars[j],
                    ]
                )
                ...
                self.gates_generator.if_then_gate(
                    cur_path_forall_equality_output_gate,
            ...
            self.encoding.append(
                [
                    "# Constratins for position "
                    + str(self.safe_max_path_length - 1)
                    + " :"
                ]
            )
            self.encoding.append(
            ...
            self.gates_generator.complete_equality_gate(
                self.goal_path_variables[self.safe_max_path_length - 1],
            ...
            self.gates_generator.if_then_gate(
                self.black_position_boolean_vars[j], self.gates_generator.output_gate
            )
            goal_step_output_gates.append(self.gates_generator.output_gate)
        ...
        for i in range(self.safe_max_path_length):
            lsc.add_circuit(
                self.gates_generator,
                self.goal_path_variables[i],
                self.parsed.num_positions,
            )
        ...
        for i in range(self.safe_max_path_length):
            self.encoding.append(
                [
                    "# Equality clause for the white forall path variables and white exists variables: "
                ]
            )
            self.gates_generator.complete_equality_gate(
                self.forall_white_goal_path_variables[i],
                self.exists_white_goal_path_variables[i],
            )
            goal_step_output_gates.append(self.gates_generator.output_gate)
        ...
        for i in range(self.safe_max_path_length - 1):
            self.encoding.append(
                [
                    "# Equality clause for the current white exists path variables and forall position variables: "
                ]
            )
            self.gates_generator.complete_equality_gate(
                self.exists_white_goal_path_variables[i], self.forall_position_variables
            )
            cur_path_forall_equality_output_gate = self.gates_generator.output_gate
            ...
            self.encoding.append(
                ["# Equality clause for the neighbours and white neighbour variables: "]
            )
            for j in range(4):
                self.gates_generator.complete_equality_gate(
                    self.neighbours[j], self.white_goal_neighbours[i][j]
                )
                step_equality_output_gates.append(self.gates_generator.output_gate)
            # Equality of the boolean variables can also be added:
            self.gates_generator.complete_equality_gate(
                self.boolean_vars, self.white_goal_boolean_vars[i]
            )
            step_equality_output_gates.append(self.gates_generator.output_gate)

            # Equality for the goal position variables can also be added:
            self.gates_generator.complete_equality_gate(
                self.predicate_variables[-1], self.white_goal_position_values[i]
            )
            step_equality_output_gates.append(self.gates_generator.output_gate)
        ...
        self.gates_generator.complete_equality_gate(
            self.exists_white_goal_path_variables[-1], self.forall_position_variables
        )
        last_equality_if_clause = self.gates_generator.output_gate

        self.encoding.append(
            [
                "# Equality clause for the white position vars and goal position variables: "
            ]
        )
        self.gates_generator.complete_equality_gate(
            self.white_goal_position_values[-1], self.predicate_variables[-1]
        )
        ...
        for i in range(self.safe_max_path_length):
            self.encoding.append(
                ["# The goal position is occupied and the color is white: "]
            )
            self.gates_generator.and_gate(
                [
                    self.white_goal_position_values[i][0],
                    self.white_goal_position_values[i][1],
                ]
            )
        ...
        for j in range(4):
            ...
            for i in range(self.safe_max_path_length - 1):
                ...
                self.encoding.append(
                    ["# Equality between nieghbour and next white position:"]
                )
                self.gates_generator.complete_equality_gate(
                    self.white_goal_neighbours[i][j],
                    self.exists_white_goal_path_variables[i + 1],
                )
        ...
        self.gates_generator.if_then_gate(
            if_valid_restricted_output_gate, final_white_output_gate
        )
        goal_step_output_gates.append(self.gates_generator.output_gate)
        #'''
    ...
    def generate_restricted_black_moves(self):
        ...
        for i in range(self.parsed.depth):
            if i % 2 == 0:
                ...
                lsc.add_circuit(
                    self.gates_generator,
                    self.move_variables[i],
                    self.parsed.num_available_moves,
                )
                step_restricted_black_output_gates.append(
                    self.gates_generator.output_gate
                )
    ...
    def generate_restricted_white_moves(self):
        ...
        for i in range(self.parsed.depth):
            if i % 2 == 1:
                ...
                step_restricted_white_output_gates.append(
                    self.gates_generator.output_gate
                )
    ...
    def generate_final_gate(self):
        ...
        if self.parsed.num_available_moves != int(math.pow(2, self.num_move_variables)):
            self.gates_generator.and_gate(
                [
                    self.restricted_black_gate,
                    self.initial_output_gate,
                    self.transition_output_gate,
                    self.goal_output_gate,
                ]
            )
        else:
            self.gates_generator.and_gate(
        ...
        if (
            self.parsed.num_positions != int(math.pow(2, self.num_position_variables))
            and self.parsed.args.restricted_position_constraints == 1
        ):
            temp_if_condition_gates.append(self.restricted_positions_gate)

        # Adding restriction white gate to if condition if enabled:
        if (
            self.parsed.num_available_moves != int(math.pow(2, self.num_move_variables))
            and self.parsed.args.forall_move_restrictions == "out"
        ):
        ...
        if len(temp_if_condition_gates) != 0:
            ...
            self.gates_generator.if_then_gate(
                self.gates_generator.output_gate, temp_and_output_gate
            )
    ...
    def __init__(self, parsed):
        ...
        for i in range(self.safe_max_path_length):
            self.goal_path_variables.append(
                self.encoding_variables.get_vars(self.num_position_variables)
            )
        ...
        for i in range(self.safe_max_path_length):
            self.forall_white_goal_path_variables.append(
                self.encoding_variables.get_vars(self.num_position_variables)
            )

        self.exists_white_goal_path_variables = []
        for i in range(self.safe_max_path_length):
            self.exists_white_goal_path_variables.append(
                self.encoding_variables.get_vars(self.num_position_variables)
            )
        ...
        self.forall_position_variables = self.encoding_variables.get_vars(
            self.num_position_variables
        )
        ...
        if parsed.args.debug == 1:
            ...
            print(
                "Down, side, diagonal, and antidiagonal variables: ", self.boolean_vars
            )
